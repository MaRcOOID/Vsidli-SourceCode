			...
			case 'PrototipoFondo':
				{
					...
					var lugar:FlxSprite = new FlxSprite(-340, -150).loadGraphic(Paths.image('PrototipoFondo'));
					lugar.setGraphicSize(Std.int(lugar.width * 2.5));
					lugar.updateHitbox();
					lugar.antialiasing = true;
					lugar.scrollFactor.set(0.9, 0.9);
					lugar.active = false;
					add(lugar);
					

				}
				case 'LugarReDibujado':
				{

			    curStage = 'unlugar2';
					var bg:FlxSprite = new FlxSprite(-12414, -200).loadGraphic(Paths.image('stageback'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					var lugar:FlxSprite = new FlxSprite(-340, -150).loadGraphic(Paths.image('lugar'));
					lugar.setGraphicSize(Std.int(lugar.width * 2.5));
					lugar.updateHitbox();
					lugar.antialiasing = true;
					lugar.scrollFactor.set(0.9, 0.9);
					...
					var suelo:FlxSprite = new FlxSprite(-320, 740).loadGraphic(Paths.image('suelo'));
					suelo.setGraphicSize(Std.int(suelo.width * 2.5));
					suelo.updateHitbox();
					suelo.antialiasing = true;
					suelo.scrollFactor.set(0.9, 0.9);
					suelo.active = false;
					add(suelo);

				}
				case 'LugarRemasterizado':
				{
	
					curStage = 'Remastered';
					defaultCamZoom = 0.5;

					if (PlayState.SONG.song.toLowerCase () == 'nevergonna')
					defaultCamZoom = 1.0;
					

					var lugar :FlxSprite = new FlxSprite(-1750, -650).loadGraphic(Paths.image('Remastered/FondoRemastered'));
					lugar.setGraphicSize(Std.int(lugar.width * 2.5));
					lugar.updateHitbox();
					lugar.antialiasing = true;
					lugar.scrollFactor.set(0.9, 0.9);
					...
					var semaforo:FlxSprite = new FlxSprite(250, 140).loadGraphic(Paths.image('Remastered/Semaforo'));
					semaforo.setGraphicSize(Std.int(semaforo.width * 1.5));
					semaforo.updateHitbox();
					semaforo.antialiasing = true;
					semaforo.scrollFactor.set(0.9, 0.9);
					semaforo.active = false;
					add(semaforo);
//offset reglas
...
//reste pa arriva
//sume pa abajo
					var edificio:FlxSprite = new FlxSprite(50, 50);
					edificio.frames = Paths.getSparrowAtlas('Remastered/Edificio');
					edificio.animation.addByPrefix('luces', 'Edificio', 24);
					edificio.animation.play('luces');
					edificio.scrollFactor.set(0.8, 0.9);
					edificio.scale.set(1, 1);
					add(edificio);

	
					var suelo:FlxSprite = new FlxSprite(-650, 780).loadGraphic(Paths.image('Remastered/SueloRemastered'));
					suelo.setGraphicSize(Std.int(suelo.width * 1.5));
					suelo.updateHitbox();
					suelo.antialiasing = true;
					suelo.scrollFactor.set(0.9, 0.9);
					suelo.active = false;
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					new FlxTimer().start(1, function(tmr:FlxTimer)
						{
							if(curSong.toLowerCase() == "nevergonna"){

								LoadingState.loadAndSwitchState(new VideoState('assets/videos/transformacion.webm', new PlayState()), true);
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curBeat % 41 == 1  && curSong == 'nevergonna')
			{
				boyfriend.playAnim('hey', true);
			}
		if (curBeat % 227 == 1  && curSong == 'nevergonna')
			{
				dad.playAnim('Reaccion-a-ser-troleado', true);
			}
	    ...
	    if (curBeat == 7 && SONG.song.toLowerCase() == 'ultimatebattle')
           {
            fantasma = new FlxTrail(boyfriend, null, 4, 24, 0.6, 0.9);
            add(fantasma);

		   }
		   
		   if (curBeat == 120 && SONG.song.toLowerCase() == 'ultimatebattle')
			{
			 fantasma = new FlxTrail(dad, null, 4, 24, 0.6, 0.9);
			 add(fantasma);
